...
size_t fs_read(int fd, void *buf, size_t len) {
	...
	size_t left_len=file_table[fd-3].size - files[fd].offset;
	...
	if(left_len==0)
		...
		ide_read(buf,file_table[fd-3].disk_offset + files[fd].offset,left_len);
		files[fd].offset+=left_len;
		return left_len;
	}
...
off_t fs_lseek(int fd, off_t offset, int whence) {
	...
	switch(whence){
		...
		case SEEK_CUR:{
			uint32_t tmp_offset=files[fd].offset+offset;
			if(tmp_offset>file_table[fd-3].size)
				return -1;
			else
				files[fd].offset=tmp_offset;
			break;
		}
		case SEEK_END:{
			if(offset>0)
